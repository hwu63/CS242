...
void print_stats(int result, alloc_stats_t *stats, double total_time);
...
void print_stats(int status, alloc_stats_t *stats, double total_time) {
  ...
  if (result == 0 && !abnormal_exit)
    ...
    printf("[mcontest]: STAUS: FAILED. SIGNAL=(%d)\n", signum);
  else
